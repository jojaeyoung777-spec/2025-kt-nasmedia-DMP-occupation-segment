"""
전국 고등학교 데이터 수집 (날짜별 버전 관리)
안전지도 API IF_0035 (학교 - 초,중,고,기타) 사용
...
def collect_high_schools():
    """
    안전지도 API를 통해 전국 고등학교 데이터 수집
    API: IF_0035 (학교 - 초,중,고,기타)

    Returns:
        DataFrame: 고등학교 데이터
    """
    print("=" * 70)
    print("[1/1] 고등학교 데이터 수집 (API: IF_0035)")
    print("=" * 70)
    print()
    ...
    print(f"API 엔드포인트: {APIConfig.SAFEMAP_HIGH_SCHOOL_URL}")
    ...
    while True:
        ...
        try:
            response = requests.get(
                APIConfig.SAFEMAP_HIGH_SCHOOL_URL,
                params=params,
                timeout=30,
                verify=False
            )
            ...
            for item in items:
                fcltynm_elem = item.find('fcltynm')
                if fcltynm_elem is not None:
                    name = fcltynm_elem.text if fcltynm_elem.text else ''

                    # 고등학교만 필터링
                    if '고등학교' in name or '고교' in name:
                        fcltycd = item.find('fcltycd')
                        lnmadr = item.find('lnmadr')
                        latitude = item.find('latitude')
                        longitude = item.find('longitude')

                        all_schools.append({
                            'fac_cd': fcltycd.text if fcltycd is not None and fcltycd.text else '',
                            'fac_nm': name,
                            'all_addr_nm': lnmadr.text if lnmadr is not None and lnmadr.text else '',
                            'lat': latitude.text if latitude is not None and latitude.text else '',
                            'lon': longitude.text if longitude is not None and longitude.text else ''
                        })

            if (page % 10 == 0) or (page == 1):
                print(f"페이지 {page:3d} 완료 - 누적: {len(all_schools):4d}개")
    ...
    df = pd.DataFrame(all_schools) if all_schools else pd.DataFrame(
        columns=['fac_cd', 'fac_nm', 'all_addr_nm', 'lat', 'lon']
    )

    print()
    print(f"[OK] 총 {len(df)}개 고등학교 수집 완료")
    print()
...
def main():
    ...
    raw_filename = f"고등학교_raw_{today}.csv"
    raw_path = PathConfig.RAW_DATA_DIR / raw_filename

    try:
        # 0. API에서 고등학교 데이터 수집
        df = collect_high_schools()
        ...
        # 이전 날짜 파일들 삭제
        cleanup_old_raw_files("고등학교", raw_path)
